...
public class CheckTrueFalse {
    ...
    public static final String FILE_OF_RESULT = "result.txt";

    public static int MONSTER_M[][] = new int[5][5];
    public static int PIT_P[][] = new int[5][5];
    public static int STENCH_S[][] = new int[5][5];
    public static int BREEZE_B[][] = new int[5][5];
    ...
    public static ArrayList<String> symbol_2;
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // load the additional knowledge base
            System.out.println("loading the additional knowledge base ...");
            ...
            while ((buffer = inputStream.readLine()) != null) {
                if (!(buffer.startsWith("#") || (buffer.equals("")))) {
                    checkAndInitializeSymbolValue(buffer); 
                    LogicalExpression subExpression = readExpression(buffer);
                    knowledge_base.setSubexpression(subExpression);
                }
            }
        ...
        symbol_2 = new ArrayList<String>(symbols);
        ...
        boolean TT_Entails_Negation_Alpha = TT_Entails(knowledge_base, statement, symbol_2, model);
     ...
     * */
    private static boolean PL_TRUE(LogicalExpression logical_statement, HashMap<String, Boolean> model, boolean isComputingNegationStatement) {
        // TODO Auto-generated method stub
    ...
    private static void createUnknownSysmbolList() {
        ...
        for (int i = 1; i < NUM_ROWS_COLS; i++) {
            for (int j = 1; j < NUM_ROWS_COLS; j++) {
                if (MONSTER_M[i][j] == UNKNOWN) {
    ...
    private static void assign_default_symbol_values() {
        ...
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                STENCH_S[i][j] = UNKNOWN;
            }
        }
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                BREEZE_B[i][j] = UNKNOWN;
            }
        }
    }
    ...
    private static void print_symbol_values() {
        ...
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                System.out.println("M_" + i + "_" + j + " = " + MONSTER_M[i][j]);
            }
        }
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                System.out.println("P_" + i + "_" + j + " = " + PIT_P[i][j]);
            }
        }
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                System.out.println("S_" + i + "_" + j + " = " + STENCH_S[i][j]);
            }
        }
        for (int i = 1; i < 5; i++) {
            for (int j = 1; j < 5; j++) {
                System.out.println("B_" + i + "_" + j + " = " + BREEZE_B[i][j]);
            }
        }
    }
    ...
    private static void checkAndInitializeSymbolValue(String line) {
        ...
        symbol_literals = symbol.split(UNDER_SCORE);
        ...
        if (symbol_initials.equals("M")) {
            MONSTER_M[pos_x][pos_y] = values_to_assign;
        } else if (symbol_initials.equals("P")) {
            PIT_P[pos_x][pos_y] = values_to_assign;
        } else if (symbol_initials.equals("S")) {
            STENCH_S[pos_x][pos_y] = values_to_assign;
        } else if (symbol_initials.equals("B")) {
            BREEZE_B[pos_x][pos_y] = values_to_assign;
        } else {
    ...
    public static boolean getValueFromArray(String symbol) {
        ...
        if (symbol_initials.equals("M")) {
            if (MONSTER_M[pos_x][pos_y] == TRUE) {
                return true;
            } else {
                return false;
            }
        } else if (symbol_initials.equals("P")) {
            if (PIT_P[pos_x][pos_y] == TRUE) {
                return true;
            } else {
                return false;
            }
        } else if (symbol_initials.equals("S")) {
            if (STENCH_S[pos_x][pos_y] == TRUE) {
                return true;
            } else {
                return false;
            }
        } else if (symbol_initials.equals("B")) {
            if (BREEZE_B[pos_x][pos_y] == TRUE) {
                return true;
            } else {
                return false;
            }
     ...
     * 
     * notes: i'm not sure that I need the COUNTER_VALUE
     */
    ...
    public static Vector<LogicalExpression> read_subexpressions(String input_string) {
        ...
        while (input_string.length() > 0) {
            ...
            if (input_string.startsWith("(")) {
                ...
                // find the matching ')'
                int parenCOUNTER_VALUE = 1;
                int matchingIndex = 1;
                while ((parenCOUNTER_VALUE > 0) && (matchingIndex < input_string.length())) {
    ...
    public static boolean valid_symbol(String symbol) {
        ...
        for (int COUNTER_VALUE = 0; COUNTER_VALUE < symbol.length(); COUNTER_VALUE++) {
            if ((symbol.charAt(COUNTER_VALUE) != '_') && (!Character.isLetterOrDigit(symbol.charAt(COUNTER_VALUE)))) {

                System.out.println("String: " + symbol + " is invalid! Offending character:---" + symbol.charAt(COUNTER_VALUE)
                    + "---\n");
        ...
        // the characters of the symbol string are either a letter or a digit or an UNDER_SCORE,
        // return true
        return true;
    }
     ...
     * 
     * @param TT_Entails_Alpha indicates wether KB entails statement or not
     * @param TT_Entails_Negation_Alpha indicates wether KB entails negation of statement or not
     * 
     * */
    private static void evaluateFinalResult(boolean TT_Entails_Alpha, boolean TT_Entails_Negation_Alpha) {
        ...
        printDecisionToFile(decision, FILE_OF_RESULT);
